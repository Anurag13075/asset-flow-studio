...
export const Route = createFileRoute("/library")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your screenfast asset library." },
    ],
  }),
...
function LibraryPage() {
  ...
  const visible = useMemo(
    () =>
      assets.filter((asset) => {
        ...
        const inView = active.collection
          ? asset.collection === active.collection
              ...
              : active.view === "duplicates"
                ? assets.filter(
                    (other) =>
                      ...
                      other.name.replace(/-(copy|v\d|final|alt)$/, "") === baseName,
                  ).length > 1
                : active.view === "recent"
                  ? Date.now() - Date.parse(asset.createdAt) < 30 * 864e5
                  : true;
        return (
          inView &&
          (!normalized ||
            `${asset.name} ${asset.ext} ${asset.kind} ${asset.tags.join(" ")}`
              .toLowerCase()
              .includes(normalized)) &&
          (kind === "all" || asset.kind === kind) &&
          (color === "all" || asset.color === color)
        );
      }),
  ...
  const toggleHandoffAsset = (id: string) => {
    setHandoffIds((current) =>
      current.includes(id) ? current.filter((item) => item !== id) : [...current, id],
    );
  };

  const checkoutUrl = import.meta.env.VITE_POLAR_CHECKOUT_URL as string | undefined;
  const hasAccess = paidEmail === user.email || isFreeAccessEmail(user.email);
  const startCheckout = () => {
    if (!checkoutUrl) {
      toast.error("Checkout is not configured", {
        description: "Add VITE_POLAR_CHECKOUT_URL to your environment.",
      });
      return;
    }
  ...
  return (
    <div className="flex h-screen min-h-[620px] overflow-hidden bg-background">
      <Sidebar
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <button
                ...
                className="hidden items-center gap-2 rounded-lg border border-border bg-surface px-3 py-1.5 text-xs text-muted-foreground sm:flex"
              >
                <Search className="size-3.5" />
                Search vault{" "}
                <kbd className="ml-3 rounded border border-border px-1 font-mono text-[10px]">
          ...
          {tab === "insights" ? (
            ...
            <>
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-primary">
                    {active.collection
                      ? collections.find((c) => c.id === active.collection)?.name
                      : active.view === "all"
                        ? "Workspace"
                        : active.view}
                  </p>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`rounded-lg border p-2 ${layout === "grid" ? "border-primary/50 bg-secondary" : "border-border text-muted-foreground"}`}
                  ...
                  <button
                    ...
                    className={`rounded-lg border p-2 ${layout === "list" ? "border-primary/50 bg-secondary" : "border-border text-muted-foreground"}`}
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                <Filter value={kind} onChange={setKind} options={["all", ...KINDS]} />
                <Filter
                  value={color}
                  onChange={setColor}
                  options={["all", "amber", "crimson", "teal", "lime", "violet", "slate", "ink"]}
                />
                <button
              ...
              <HandoffBuilder
                assets={visible}
                selectedIds={handoffIds}
                onToggle={toggleHandoffAsset}
                onClear={() => setHandoffIds([])}
              />
              <div className="mt-5">
                {visible.length === 0 ? (
                  ...
                  <AssetGrid
                    ...
                    onToggleFavorite={(a) => updateAsset(a.id, { favorite: !a.favorite })}
                  />
                ) : (
                  <AssetList
                    assets={visible}
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-md px-3 py-1.5 text-xs font-medium ${active ? "bg-secondary text-foreground" : "text-muted-foreground"}`}
    >
      {children}
    </button>
  );
  ...
  return (
    <label className="relative">
      <select
        ...
        className="appearance-none rounded-lg border border-border bg-surface py-2 pl-3 pr-8 text-xs capitalize outline-none focus:border-primary/60"
      >
        {options.map((option) => (
  ...
  return (
    <main className="grid min-h-screen place-items-center px-6 py-16">
      <div className="surface-panel w-full max-w-md rounded-2xl p-8 text-center">
        ...
        <p className="mt-6 text-[11px] font-semibold uppercase tracking-widest text-primary">
          screenfast lifetime access
        </p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">Your vault is ready.</h1>
        <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
          Unlock the full local-first library for a single $2 payment. No subscription, no usage
          limits.
        </p>
        <ul className="mt-6 space-y-2 text-left text-sm text-muted-foreground">
          {[
            "Unlimited assets and collections",
            "Duplicate radar and insights",
        ...
        <button
          onClick={onCheckout}
          className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground hover:-translate-y-0.5"
        >
          Unlock for $2 <Sparkles className="size-4" />
        </button>
